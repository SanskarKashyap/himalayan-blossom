(function () {
  ...
  const fallbackBase = `${window.location.origin.replace(/\/$/, '')}/api`;
  const CONFIG = {
    apiBaseUrl: (window.APP_API_BASE_URL || fallbackBase).replace(/\/$/, ''),
    googleClientId: (window.APP_GOOGLE_CLIENT_ID || '').trim(),
    googleRedirectUri:
      (window.APP_GOOGLE_REDIRECT_URI || `${fallbackBase}/auth/google/`).trim(),
  };
